...
// const Login = ({ isUserAuthenticated,setUsername }) => {
...
const Login = ({ isUserAuthenticated, setUsername }) => {
    ...
    const loginUser = async () => {
        ...
        if (response.isSuccess) {
            ...
            setLogin(loginInitialValues);
            navigate('/');
        } else if (response.errorCode === 'INVALID_USERNAME') {
            // Check if the error is due to an invalid username
            showError('Username is invalid. Please try again.');
            ...
            showError('Invalid username or password! please try again later');
        }
    }
    ...
    return (
        <Component>
            <Grid container>
                <Grid item xs={4}>
                    <Image src={leftImageURL} alt="left side" style={{
                        height: '320px',
                        width: '320px',
                        display: 'block',
                        padding: 'auto 10',
                ...
                <Grid item xs={4} >
                    <Box display="flex" flexDirection="column" justifyContent="center" width="40vw" height="100%" >
                        <Image src={rightImageURL} alt="right side" />
                        {
                            account === 'login' ?
                                <Wrapper>
                                    <TextField variant="standard" value={login.username} onChange={(e) => onValueChange(e)} name='username' label='Enter Username'  error={!!error} />
                                    <TextField variant="standard" value={login.password} onChange={(e) => onValueChange(e)} name='password' label='Enter Password'   error={!!error}/>
                                    {error && <Error>{error}</Error>}
                                    <LoginButton variant="contained" onClick={loginUser}>Login</LoginButton>
                                    {/* <LoginButton variant="contained" onClick={() => loginUser()} >Login</LoginButton> */}
                                    <Text style={{ textAlign: 'center' }}>OR</Text>
